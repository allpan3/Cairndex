...
that stored server only when Tauri is present. The backend permits only the
three packaged Tauri custom-protocol origins and the exact Vite origin used by
`tauri dev`. No source media or library metadata is stored in the shell.
...
src/
  ...
  desktop/    Tauri store/bootstrap, menu-event bridge, close-beacon guard
