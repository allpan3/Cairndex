...
| E2     | **Library ownership lease + local-server sidecar** ([ADR-0018](../adr/0018-library-ownership-lease-and-local-server.md), proposed 2026-07-19) | Enforces one server per library and lets the desktop client open local library folders without server administration. The server-side lease lands first (it hardens the NAS deployment on its own); the shell's sidecar + connections model follows (plan 3 D6). Slots after D5, alongside/before write mode — the lease becomes a precondition for write-mode operations |
