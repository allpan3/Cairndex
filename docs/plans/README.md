...
> **shipped** — v0.1.0 (release withdrawn in the 2026-08-09 recreation)
> is public (2026-07-28). Plan 3 is complete, plan 1 has everything but its deferred
...
| C ✅   | Plan 1 **M9 player interaction polish**, then **M12 hover video preview** (both recomposed 2026-07-11 — plan 1 §11/§13) | Merged as #11 and #12. Every later client inherits the same SPA behavior. Range loop remains in M11, video adjustments + slideshow deferred                                                          |
...
| H2 ✅  | **Publish v0.1.0** — the tag, and the owner's pass on a genuinely downloaded build | **Published 2026-07-28**: tag `v0.1.0` at `0821890`, one Apple Silicon DMG + checksum + notices. Phase G had built and proved the pipeline, but nothing was published — the D7 verification deferred *until after write mode* was this step. Both owner-only checks passed on a real download: the Gatekeeper first-launch path, and drag-out to Finder from an SMB library (a copy, by design — the shell never removes the source) |
| I (next) | **Android client** (plan 2 T1–T7)                                                                                       | Largest new surface; by then every server API it needs exists and is proven (pairing landed in phase D)                                                                                            |
