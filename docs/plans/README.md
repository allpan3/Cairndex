...
| H2 ✅  | **Publish v0.1.0** — the tag, and the owner's pass on a genuinely downloaded build | **Published 2026-07-28**: tag `v0.1.0` at `2036cc2`, one Apple Silicon DMG + checksum + notices. Phase G had built and proved the pipeline, but nothing was published — the D7 verification deferred *until after write mode* was this step. Both owner-only checks passed on a real download: the Gatekeeper first-launch path, and drag-out to Finder from an SMB library (a copy, by design — the shell never removes the source) |
