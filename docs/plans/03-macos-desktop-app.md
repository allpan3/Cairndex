...
| D7 (in progress) | First public release | The release pipeline, not new product surface. **Done:** the static ffmpeg/ffprobe pin ([ADR-0019](../adr/0019-open-source-distribution-model.md) §3) — both macOS architectures, verified static/redistributable/libx264/signed, with a per-platform checksum gate and CI building and smoke-testing the bundled binary; the README install section covering the Open Anyway first launch; the GPL source offer (`THIRD-PARTY-NOTICES.md`); and the quarantined-build check, which found the bundle shipping an **invalid** signature (no `_CodeSignature`) and fixed it with `signingIdentity: "-"` (§4 correction). the per-architecture release workflow (`release.yml`), building both arches natively because PyInstaller cannot cross-compile, and verified locally by building, signing and running a full Intel app under Rosetta. **Remaining:** a real CI run of that workflow (it triggers on tags, so nothing has executed it); an owner pass on a genuinely downloaded build; the `pillow-heif`/libheif LGPL notice question. Tauri's updater rides here or later — it is unblocked, not required, though the per-update Gatekeeper repetition (§4 amendment) raises its value |
