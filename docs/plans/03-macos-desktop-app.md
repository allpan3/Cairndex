...
| D7 ✅ | First public release | The release pipeline, not new product surface. **Static ffmpeg/ffprobe pin** ([ADR-0019](../adr/0019-open-source-distribution-model.md) §3) — both macOS architectures, each verified static, redistributable (the licensing check disqualified the obvious candidate), libx264-capable and signed, behind a per-platform checksum gate. **Release workflow** (`release.yml`), tag-triggered, drafting a release with a DMG, its `.sha256` and the GPL notices; run for real at `v0.1.0` and green. Shipped as a two-architecture matrix, then cut to **Apple Silicon only** (owner, 2026-07-23) once that run measured Intel at ~4× the compile time for an artifact nobody needs — Intel stays pinned and locally buildable, one commented matrix entry away. **README install section** for the Open Anyway first launch, including that it repeats per update. **GPL source offer** in `THIRD-PARTY-NOTICES.md`, with each build's configure line and component versions committed rather than linked. Two defects found by doing it rather than planning it: the bundle shipped an **invalid** signature (no `_CodeSignature`, fixed with `signingIdentity: "-"` — §4 correction), and the HEIC dependency linked a **GPL x265 encoder** the product never calls (swapped to decode-only `pi-heif`, enforced by a spec exclude and a build gate). **Deferred by owner to after write mode:** an owner pass on a genuinely downloaded build — everything short of a browser round trip is verified. Tauri's updater rides later; it is unblocked, not required, though the per-update Gatekeeper repetition (§4 amendment) raises its value |
