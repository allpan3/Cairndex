...
| W6 ✅ | Hardening | EXDEV/case-only edge cases, retention config, journal history UI, deployment/backup docs, perf pass on bulk ops. **Landed 2026-07-26 (PR #33)**, closing the unblocked write-mode track — W2 is the only slice left and it waits on plan 1 M11. Landed: **EXDEV** cross-device moves as copy-then-delete (owner-approved), whose crash window needed the reconciler to learn a new state, with an explicit **marker file** as the evidence rather than inference from size+mtime; **case-only renames**; `CAIRNDEX_TRASH_RETENTION_DAYS`; the deployment/backup section on libraries spanning more than one filesystem (what the EXDEV fallback costs, and that an interruption leaves a duplicate, never a hole); bundle **delete with files** (the last W4 remnant); and a hardening fix the PR #30 review had flagged — the desktop importer built its upload URL by `format!`, so a library id carrying `..`, `?`, `#` or an extra `/` restructured it. **Not done, with reasons:** the **journal history UI** (feature-sized — a new surface with its own layout and empty/error states, not a hardening fix) and the **perf pass on bulk ops** (needs a representative large library, which is the owner's to point at; AGENTS.md asks for recorded baselines rather than claims). Two more were judged **not worth doing** rather than deferred: auto-suffixing within a move batch, and the two move-undo manual-recovery cases from the W3 review |
