...
test('places a folder stem control on the deepest matching container row', async () => {
  ...
  const innerRow = screen
    .getByRole('button', { name: 'Rename collection suggestion Ada Larson' })
    .closest('.grp-row') as HTMLElement

  expect(
    within(outerRow).queryByLabelText('Stem matching for Western/Ada Larson'),
  ).not.toBeInTheDocument()
  expect(within(innerRow).getByLabelText('Stem matching for Western/Ada Larson')).toBeVisible()
})
