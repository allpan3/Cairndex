...
test('switches one addition row between an existing and a new bundle', async ({ page }) => {
  ...
  await expect(page.getByText('Add to Sky, Sand, Sea & Salt - 4K')).toBeVisible()
  ...
  await page
    .getByRole('button', { name: 'Rename bundle suggestion Surf On The Ridge - 4K' })
    .dblclick()
  ...
  await page.getByRole('button', { name: 'Add to Sky, Sand, Sea & Salt - 4K instead' }).click()
  await expect(page.getByText('Add to Sky, Sand, Sea & Salt - 4K')).toBeVisible()
